...
char** crea_tabla(FILE* fd, int No_estados)
{
    ...
    {
        tabla[j]=(char*) calloc (5, sizeof(char));
    }
...
void verifica_cadena(char** tabla, int No_estados, FILE* fd_cadenas)
{
    ...
    while(fgets(verifica, 100, fd_cadenas) != NULL)
    {
        ...
        if(verifica[0]=='E') {
            if(!((verifica[1]=='a')||(verifica[1]=='b')||(verifica[1]=='c')))
            {
                printf("Cadena \"%s\" aceptada (E)\n", verifica);
                continue;
            }
            else if((verifica[1]=='a')||(verifica[1]=='b')||(verifica[1]=='c'))
            {
                entrada=1;
            }
        }
        ...
        {
            if(verifica[entrada]=='a') 
            {
                estadoa= *(tabla[nestado(estadoa, tabla, No_estados)]+2);
            } 
            else if(verifica[entrada] == 'b')
            {
                estadoa=*(tabla[nestado(estadoa, tabla, No_estados)]+3);
            }
            else if(verifica[entrada] == 'c')
            {
                estadoa= *(tabla[nestado(estadoa, tabla, No_estados)]+4);
            }
            entrada++;
        }
